...
namespace HexFileTools
{
    class HexFile
    {
        ...
        public int hexStartSegmentAddress = 0;

        // variables for version number found in the hexfile
        public int versionHexfileMajor = -1;
        public int versionHexfileMinor = -1;

        public int cBUFFERSIZE = 1024*1024;
        ...
        public int ReadFile(string path)
        {
            ...
            byte[] hexBuffer = new byte[1024];  // buffer for one line (record) of the hex file
            ...
            UInt16 hexExtendedLinearAddress = 0;
            UInt16 hexStartSegmentAddress_Segment = 0;
            UInt16 hexStartSegmentAddress_Offset = 0;

            // some flags
            bool flagEndOfFile = false;
            bool flagEndOfFileRecordMissing = false;
            bool GetFirstOffset = true;
            ...
            do
            {
                ...
                do
                {
                    ...
                    if ((hexBuffer[readIndex] == '\r') || (hexBuffer[readIndex] == '\n'))  // if it's a carriage return or new line, we exit the do while loop
                        break;
                ...
                if (hexBuffer[0] == ':' && flagEndOfFile == false)
                {
                    ...
                    // if hex file does not start at address zero, we set hexStartSegmentAddress to first hex record address
                    // need to think about that, if it's the best case for all szenarios...
                    if ((hexStartSegmentAddress == 0) && GetFirstOffset == true)
                    {
                        hexStartSegmentAddress = hexAddress;
                    ...
                    switch (hexRecordType)
                    {
                        case 0:  // normal data record
                            ...
                            {  // increment by two
                                ...
                                buffer[hexAddress + (hexExtendedSegmentAddress * 16) + (hexExtendedLinearAddress * 65536) + (i >> 1)] = dataByte;
                            }

                            // in case the hex file is unordered, 
                            if (bufferLength < hexAddress + (hexExtendedSegmentAddress * 16) + (hexExtendedLinearAddress * 65536) + (i >> 1))
                            {
                                bufferLength = hexAddress + (hexExtendedSegmentAddress * 16) + (hexExtendedLinearAddress * 65536) + (i >> 1);
                            }
                        ...
                        case 2:  // extended segment address record
                            ...
                            // clear other extended address records
                            hexExtendedLinearAddress = 0;
                            break;

                        case 3:  // start segment address record
                            hexStartSegmentAddress_Segment = (UInt16)(asciiToHex2(hexBuffer[9], hexBuffer[10]) << 8);  // get high byte of segment
                            myChecksum += (byte)(hexStartSegmentAddress_Segment >> 8);  // compute owm checksum
                            hexStartSegmentAddress_Segment += (UInt16)(asciiToHex2(hexBuffer[11], hexBuffer[12]));  // get low byte of segment
                            myChecksum += (byte)(hexStartSegmentAddress_Segment & 0xff);  // compute owm checksum
                            hexStartSegmentAddress_Offset = (UInt16)(asciiToHex2(hexBuffer[13], hexBuffer[14]) << 8);  // get high byte of offset
                            myChecksum += (byte)(hexStartSegmentAddress_Offset >> 8);  // compute owm checksum
                            hexStartSegmentAddress_Offset += asciiToHex2(hexBuffer[15], hexBuffer[16]);  // get low byte of offset
                            myChecksum += (byte)(hexStartSegmentAddress_Offset & 0xff);  // compute owm checksum
                            hexStartSegmentAddress = (hexStartSegmentAddress_Segment * 16) + hexStartSegmentAddress_Offset;
                            break;

                        case 4:  // extended linear address record
                            hexExtendedLinearAddress = (UInt16)(asciiToHex2(hexBuffer[9], hexBuffer[10]) << 8);  // get high byte of extended linear address
                            myChecksum += (byte)(hexExtendedLinearAddress >> 8);  // compute owm checksum
                            hexExtendedLinearAddress += asciiToHex2(hexBuffer[11], hexBuffer[12]);  // get low byte of extended linear address
                            myChecksum += (byte)(hexExtendedLinearAddress & 0xff);  // compute owm checksum
                            // clear other extended address records
                            hexExtendedSegmentAddress = 0;
                            break;
        ...
        public void WriteFile(string path)
        {
            ...
            for (int hexFileLineCounter = 0; hexFileLineCounter <= ((bufferLength - 1) / hexFileByteCount); ++hexFileLineCounter)
            {
                ...
                for (int hexFileDataByteCounter = 0; hexFileDataByteCounter < 16; ++hexFileDataByteCounter)
                {
                    ...
                    byte c = buffer[flashBufferAddress + hexFileDataByteCounter];
        ...
        // read the firmware version from the hexfile
        public bool ReadHexfileVersion()
        {
            byte[] patternToFind = { (byte)'F', (byte)'W', (byte)'_', (byte)'V', (byte)'E', (byte)'R', (byte)'S', (byte)'I', (byte)'O', (byte)'N', (byte)':' };

            if (patternToFind.Length > bufferLength)
                return false;

            for (int i = 0; i < bufferLength - patternToFind.Length; i++)
            {
                bool found = true;
                for (int j = 0; j < patternToFind.Length; j++)
                {
                    if (buffer[i + j] != patternToFind[j])
                    {
                        found = false;
                        break;
                    }
                ...
                if (found)
                {
                    i += patternToFind.Length;
                    versionHexfileMajor = (buffer[i + 0] - '0') * 10 + (buffer[i + 1] - '0');
                    versionHexfileMinor = (buffer[i + 3] - '0') * 10 + (buffer[i + 4] - '0');

                    return true;
                }
                else
                {
                    versionHexfileMajor = -1;
                    versionHexfileMinor = -1;
                }
            }
            return false;
        }
